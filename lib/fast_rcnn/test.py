...
def _get_image_blob(im):
    ...
    for target_size in cfg.TEST.SCALES:
        ...
        im_shapes.append(np.array(im.shape[1::-1], dtype=np.float32).reshape(1, -1))

    blob = processed_ims

    return blob, np.array(im_scale_factors), im_shapes
...
def _get_blobs(im, rois):
    ...
    blobs = {'data' : None, 'rois' : None, 'shapes' : None}
    blobs['data'], im_scale_factors, blobs['shapes'] = _get_image_blob(im)
    ...
    for i in xrange(num_images):
        for j in xrange(imdb.num_classes):
            ...
            with open(cls_file, 'a') as f:
                ...
                f.write(imdb.image_index[i]+' '+str(nms_dets[j][i][tmp, 0])+' '
                    +str(nms_dets[j][i][tmp, 1])+' '+str(nms_dets[j][i][tmp, 2])+' '
